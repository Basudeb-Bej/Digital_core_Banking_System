// front-end/src/pages/userDashboard/WithdrawalHistory.jsx
      ...
      <Card className="shadow-sm border-0 rounded-4 p-4">
        <div className="d-flex flex-wrap justify-content-between align-items-center mb-3 gap-2">
          <h4 className="fw-bold text-dark mb-0 d-flex align-items-center gap-2">
            <FaWallet className="text-danger" /> Withdrawal History
          </h4>
              ...
              {filtered.length > 0 ? (
                filtered.map((item) => (
                  <tr key={item._id}>
                    ...
                    <td className="fw-semibold text-dark">{item.recipientName || "ATM Withdrawal"}</td>
                    <td><span className="font-monospace text-muted">{item.recipientAccount || "-"}</span></td>
                    <td className="text-danger fw-bold">-₹{Number(item.amount).toLocaleString("en-IN")}</td>
                    <td><Badge bg="success">Success</Badge></td>
                    <td>{item.description || "Withdrawal"}</td>
                  </tr>
                ))
              ) : (
                <tr>
